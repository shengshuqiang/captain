...
final class TimerWaterPainter {
    ...
    TimerWaterPainter(Context context, float density) {
    ...
    // 取 token RGB 并叠加运行时计算的 alpha，保留原有动态透明度逻辑。
    private int withAlpha(int resId, int alpha) {
        int base = color(resId);
    ...
    void drawWaterAndStone(Canvas canvas, int width, int height, boolean day, float progress,
        ...
        paint.setShader(new RadialGradient(width / 2f, stoneCenterY - stoneHeight * 0.24f,
                stoneWidth * 0.58f,
                color(day ? R.color.captain_timer_stone_core_day : R.color.captain_timer_stone_core_night),
                color(day ? R.color.captain_timer_stone_edge_day : R.color.captain_timer_stone_edge_night),
    ...
    private void drawStoneMarks(Canvas canvas, float centerX, float centerY, float stoneWidth,
        ...
        int traceColor = day
                ? withAlpha(R.color.captain_timer_stone_trace_day, (int) (115 * traceStrength))
                : withAlpha(R.color.captain_timer_stone_trace_night, (int) (150 * traceStrength));
        ...
        paint.setColor(color(day ? R.color.captain_timer_stone_hole_day : R.color.captain_timer_stone_hole_night));
    ...
    private void drawFallingDrop(Canvas canvas, int width, boolean day, float progress,
        ...
        paint.setColor(color(day ? R.color.captain_timer_drop_trail_day : R.color.captain_timer_drop_trail_night));
        canvas.drawOval(x - dp(3), y - dp(28 * stretch), x + dp(3), y - dp(6), paint);
        paint.setShader(new RadialGradient(x - dp(3), y - dp(7), dp(18),
                color(day ? R.color.captain_timer_drop_core_day : R.color.captain_timer_drop_core_night),
                color(day ? R.color.captain_timer_drop_edge_day : R.color.captain_timer_drop_edge_night),
                Shader.TileMode.CLAMP));
        canvas.drawOval(x - dp(7), y - dp(11 * stretch), x + dp(7), y + dp(9), paint);
        paint.setShader(null);
        paint.setColor(color(R.color.captain_timer_drop_highlight));
        ...
        if (progress >= impactStart) {
            ...
            strokePaint.setColor(withAlpha(
                    day ? R.color.captain_timer_ripple_day : R.color.captain_timer_ripple_night, alpha));
            ...
            paint.setColor(withAlpha(
                    day ? R.color.captain_timer_ripple_day : R.color.captain_timer_ripple_night, alpha / 2));
